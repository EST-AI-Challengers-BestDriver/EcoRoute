...
출발시간 옆에서 운전 요일을 선택할 수 있으며, 이 값은 DNN 입력과 주간 기록에 함께 반영됩니다. 주간 리포트는 요일별 상대 CO2 절감률, 완료 일수, 가중 총 절감률, 일평균 및 최고 절감 요일을 표시합니다. 같은 요일을 여러 번 기록하면 가장 최근 결과가 기존 값을 교체합니다. 기록은 브라우저 `sessionStorage`에만 저장되므로 새로고침에는 유지되지만 해당 탭이나 브라우저 창을 닫으면 삭제됩니다.
